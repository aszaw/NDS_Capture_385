...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int status = XSpi_Transfer(&SpiInstance, wbuf, NULL, 2);
	if (status != XST_SUCCESS) {
		xil_printf ("SPI Peripheral threw a nonzero return code %d", status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance,1);
	int status = XSpi_Transfer(&SpiInstance, wbuf, NULL, nbytes + 1);
	if (status != XST_SUCCESS) {
		xil_printf ("SPI Peripheral threw a nonzero return code: %d", status);
	}
	XSpi_SetSlaveSelect(&SpiInstance,0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	wbuf[0] = reg;
	XSpi_SetSlaveSelect(&SpiInstance,1);
	int status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, 2);
	if (status != XST_SUCCESS) {
		xil_printf ("SPI Peripheral threw a nonzero return code", status);
	}
	XSpi_SetSlaveSelect(&SpiInstance,0);
	return rbuf[1];
	/*
	...
	u8 rbuf[1];
	wbuf[0] = reg;
	int retcode = XSpi_Transfer(&SpiInstance, wbuf, rbuf, 1);
	if (retcode != 0) {
		//xil_printf ("SPI Peripheral threw a nonzero return code: %d", retcode);
	}*/
	//psuedocode:
	//select MAX3421E
	//write reg via SPI
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	wbuf[0] = reg;
	XSpi_SetSlaveSelect(&SpiInstance,1);
	int status = XSpi_Transfer(&SpiInstance, wbuf, rbuf, nbytes+1);
	if (status != XST_SUCCESS) {
		xil_printf ("SPI Peripheral threw a nonzero return code: %d", status);
	}
	...
	u8 rbuf[nbytes];
	wbuf[0] = reg;
	int retcode = XSpi_Transfer(&SpiInstance, wbuf, rbuf, nbytes);
	if (retcode != 0) {
		//xil_printf ("SPI Peripheral threw a nonzero return code: %d", retcode);
	}
	*/
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)c
	//write reg via SPI
