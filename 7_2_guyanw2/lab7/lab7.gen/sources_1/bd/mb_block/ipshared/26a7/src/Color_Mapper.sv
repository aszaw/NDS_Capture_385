...
//                       input [31:0] slaves[601],
                       ...
                       input logic [31:0] palette_reg[7], //Register for color palette
...
//	                      end
	          ...
	          assign font_addr = {sprite_code[6:0],(DrawY % 16)};
...
//                 Blue = slaves[600][4:1]; //invert font_B to background color_B
                 Red = palette_reg[FGD_IDX/2][24:21]; 
                 Green = palette_reg[FGD_IDX/2][20:17];
                 Blue = palette_reg[FGD_IDX/2][16:13]; 
                 end 
                 else begin
                        Red = palette_reg[FGD_IDX/2][12:9]; 
                        Green = palette_reg[FGD_IDX/2][8:5];
                        Blue = palette_reg[FGD_IDX/2][4:1]; 
                    end
                end
            else begin 
...
//                 Blue = slaves[600][16:13]; // Keep font_B to font color_B
                    if (BKG_IDX%2) begin
                        Red = palette_reg[BKG_IDX/2][24:21]; 
                        Green = palette_reg[BKG_IDX/2][20:17];
                        Blue = palette_reg[BKG_IDX/2][16:13]; 
                    end
                    else begin
                        Red = palette_reg[BKG_IDX/2][12:9]; 
                        Green = palette_reg[BKG_IDX/2][8:5];
                        Blue = palette_reg[BKG_IDX/2][4:1]; 
                    end
